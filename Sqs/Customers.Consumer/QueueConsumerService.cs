    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            foreach (var message in response.Messages)
            {
                ...
                var type = Type.GetType($"Customers.Customer.Messages.{messageType}");

                if (type is null)
                {
                    _logger.LogWarning($"Unknown message type: {messageType}");
                    // The message isn't being processed here so it will go back to the Queue
                    // Based on the SQS configuration, if it's sent back to the Q 3 times, it will be sent to the Dead Letter Queue and new messages can then be processed
                    continue;
                }
